...
LINEAR_QUESTION_SPECS: dict[str, QuestionSpec] = {
    ...
    "perpendicular_lines": QuestionSpec(
        question_type="perpendicular_lines",
        family="linear",
        question_template=(
            "A straight line g is perpendicular to f(x) = {equation} and passes through "
            "the point A{point_a}. Determine the equation of g."
        ),
        answer_type="equation",
        display_overrides=_policy(
            show_x_intercepts=False, show_y_intercepts=False,
            show_intersection_points=False, show_point_labels=False,
            show_additional_point_labels=True, show_grid=True,
            show_tick_labels=True, show_tick_marks=True,
            show_gradient=False, show_gradient_triangle=False,
            graph_curve_label_style="Function name only",
        ),
        ai_visible_information=("reference equation", "point A", "perpendicular relationship"),
